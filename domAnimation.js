...
function markCheckboxItem(e) {
  ...
  let progressElement = e.target.closest('.checklist-content').children[1].firstElementChild;
...
function addItemsInChecklist(checkListContent, checkboxTitle, data) {
  let checkBoxInput = document.createElement('input');
  checkBoxInput.className = 'form-check-input';
  checkBoxInput.type = 'checkbox';
  checkBoxInput.value = "";
...
function addItemsAPIInChecklist(event) {
  ...
  if(checkboxTitle.length > 0) {
    ...
    .then((data) => {
      ...
      progressbarUpdation(progressbar, itemCount, countCheckedItem);
      addItemsInChecklist(checkListContent, checkboxTitle, data);
      console.log('Success:', data.name);
    })
    .catch((error) => {
      let topParentElement = document.getElementsByClassName('list')[0];
...
function addCheckListToTheCard(data, checklistTitle) {
  let pElement = document.createElement('p');
  pElement.className = 'checklist-title';
...
function addCheckListAPI() {
  ...
  if(checklistTitle > 0) {
    ...
    .then((response) => response.json())
    .then((data) => {
      addCheckListToTheCard(data, checklistTitle);
      console.log('Success:', data.name);
    })
    .catch((error) => {
...
function addCardToTheList(data, title) {
  let cardBody = document.createElement('div');
  cardBody.className = 'card-body';
